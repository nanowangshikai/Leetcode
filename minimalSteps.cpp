...
public:
    ...
    vector<vector<int> > bfs(int x, int y, vector<string>& maze){
        ...
        while(!Q.empty()){
            ...
            if(inBound(nx,ny) && maze[nx][ny] != '#' && ret[ny][ny] == -1 ){
                    ret[nx][ny] = ret[x][y] + 1;
                    Q.push({nx,ny});
                }
            }
    ...
    int minimalSteps(vector<string>& maze) {
        ...
        vector<pair<int, int>> buttons, stones; //机关 和 石头
        int sx, sy, tx, ty; //起点 和 终点
        ...
        cout << "how many buttons ? " << nb <<endl;
        cout << "how many stones ? " << ns <<endl;
